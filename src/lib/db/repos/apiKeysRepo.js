...
export async function createApiKey(name, machineId, allowedModels = []) {
  ...
  db.run(
    ...
    [apiKey.id, apiKey.key, apiKey.name, apiKey.machineId, 1, apiKey.createdAt, stringifyJson(allowedModels)]
  );
  return apiKey;
}
...
export async function updateApiKey(id, data) {
  ...
  db.transaction(() => {
    ...
    db.run(
      ...
      [merged.key, merged.name, merged.machineId, merged.isActive ? 1 : 0, stringifyJson(merged.allowedModels || []), id]
    );
    result = merged;
  });
  return result;
